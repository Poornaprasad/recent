...
export function mapDbRowToInvoice(row: Invoice): StoredInvoice {
  return {
    ...
    saCreatedDate: row.saCreatedDate ? new Date((row.saCreatedDate as Date).getTime() * 1000) : undefined,
    saModifiedUserId: row.saModifiedUserId !== null && row.saModifiedUserId !== undefined ? Number(row.saModifiedUserId) : undefined,
    saModifiedDate: row.saModifiedDate ? new Date((row.saModifiedDate as Date).getTime() * 1000) : undefined,
    ...
    saDocumentDate: row.saDocumentDate ? new Date((row.saDocumentDate as Date).getTime() * 1000) : undefined,
