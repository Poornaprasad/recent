...
export function mapDbRowToInvoice(row: Invoice): StoredInvoice {
  return {
    ...
    approvedAt: row.approvedAt instanceof Date ? new Date(row.approvedAt.getTime() * 1000) : undefined,
    ...
    saCreatedDate: row.saCreatedDate instanceof Date ? new Date(row.saCreatedDate.getTime() * 1000) : undefined,
    saModifiedUserId: row.saModifiedUserId !== null && row.saModifiedUserId !== undefined ? Number(row.saModifiedUserId) : undefined,
    saModifiedDate: row.saModifiedDate instanceof Date ? new Date(row.saModifiedDate.getTime() * 1000) : undefined,
    ...
    saDocumentDate: row.saDocumentDate instanceof Date ? new Date(row.saDocumentDate.getTime() * 1000) : undefined,
...
export function mapInvoiceToDbRow(invoice: Partial<StoredInvoice>): Partial<Invoice> {
  ...
  return {
    ...
    approvedAt: invoice.approvedAt instanceof Date ? Math.floor(invoice.approvedAt.getTime() / 1000) as any : undefined,
    ...
    saCreatedDate: invoice.saCreatedDate instanceof Date ? Math.floor(invoice.saCreatedDate.getTime() / 1000) as any : undefined,
    saModifiedUserId: invoice.saModifiedUserId,
    saModifiedDate: invoice.saModifiedDate instanceof Date ? Math.floor(invoice.saModifiedDate.getTime() / 1000) as any : undefined,
    ...
    saDocumentDate: invoice.saDocumentDate instanceof Date ? Math.floor(invoice.saDocumentDate.getTime() / 1000) as any : undefined,
