...
export function mapDbRowToVendor(row: DbVendor): Vendor {
  return {
    ...
    w9ReceivedDate: row.w9ReceivedDate ? new Date((row.w9ReceivedDate as Date).getTime() * 1000) : undefined,
    w9ExpiryDate: row.w9ExpiryDate ? new Date((row.w9ExpiryDate as Date).getTime() * 1000) : undefined,
...
export function mapVendorToDbRow(vendor: Partial<Vendor>): Partial<DbVendor> {
  return {
    ...
    w9ReceivedDate: vendor.w9ReceivedDate ? Math.floor(vendor.w9ReceivedDate.getTime() / 1000) as any : undefined,
    w9ExpiryDate: vendor.w9ExpiryDate ? Math.floor(vendor.w9ExpiryDate.getTime() / 1000) as any : undefined,
